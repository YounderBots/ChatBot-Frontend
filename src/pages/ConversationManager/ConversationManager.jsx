...
const ConversationManager = () => {
  ...
  return (
    <Row className="g-1">
      <Col xs={12} lg={4} className="bg-light p-3 border rounded">
        ...
        <Form.Label className="mt-3 ">Intent</Form.Label>
        {intents.map((intent) => (
          <Form.Check
            key={intent}
            type="checkbox"
        ...
        <Form.Label className="mt-3">Confidence Range</Form.Label>
        ...
        <Form.Label className="mt-3">Sentiment</Form.Label>
        {[
          { label: "All", value: "All" },
          ...
          <Form.Check
            ...
            style={{ accentColor: "#0d6efd" }} // radio color
          />
        ))}
        ...
        <div className="mt-3">
          {conversations
            ...
            .map(conv => (
              <div
                key={conv.id}
                className={`d-flex align-items-center p-2 mb-2 border rounded ${selectedConversation?.id === conv.id
                  }`}
              ...
              >
                ...
                <div className="flex-grow-1">
                  <div className="d-flex justify-content-between align-items-center">
                    <span style={{ fontWeight: "bold" }}>{conv.firstMessage}</span>
                    <small className="text-muted">{conv.lastTimestamp}</small>
                  </div>
                  <div className="d-flex justify-content-between align-items-center mt-1">
                    <span className="badge" style={{ backgroundColor: "#165d9c" }}>{conv.intent}</span>
                    ...
                    <div className="d-flex align-items-center gap-1">
                      {conv.unread}
                      <span className={`badge ${conv.status === "Resolved" ? "bg-success" :
                        conv.status === "Pending" ? "bg-warning" :
                          conv.status === "Escalated" ? "bg-danger" : "bg-secondary"}
                        `}>{conv.status}</span>
                    </div>
                  </div>
                  <small className="text-muted">{conv.id}</small>
                </div>
              </div>
            ))}
        </div>
          ...
          <nav>
            <ul className="pagination pagination-sm mb-0">
              <li className={`page-item ${currentPage === 1 ? "disabled" : ""}`}>
                <Button
                  style={{ backgroundColor: "#0f3d6e", color: "white" }}
                  className="page-link"
              ...
              {[...Array(totalPages)].slice(
                ...
                return (
                  ...
                  >
                    <Button
                      style={{
                        backgroundColor: currentPage === pageNumber ? "#0f3d6e" : "",
          ...
          <>
            ...
            <div className="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">
              ...
              <div className="d-flex gap-2">
                <Button size="sm" variant="outline-primary">Export PDF</Button>
                <Button size="sm" variant="outline-primary">Export JSON</Button>
                <Button size="sm" variant="outline-warning">Flag</Button>
                <Button size="sm" variant="outline-danger">Delete</Button>
                <Button size="sm" variant="outline-success">Escalate</Button>
              </div>
            </div>
